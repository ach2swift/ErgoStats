...
struct ContractsView: View {
    ...
    @EnvironmentObject var vmContracts: ContractsViewModel
    
    @State private var showAlert = false
    
    @State private var lineWidth = 2.0
    @State private var chartColor: Color = .orange
    @State private var selectedElement: ContractsItem?
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack {
                Text("Unique contract addresses with minimum balance 1ERG / 30 days")
                    .font(.callout)
                    .foregroundColor(.secondary)
                   
                Chart(vmContracts.contracts) {
                    LineMark(
                        x: .value("Date", $0.date),
                ...
                LazyVGrid(
                    ...
                    content: {
                        ...
                        Text("4 Weeks").bold()
                        Text("1 Year").bold()
                        ForEach(vm.summaryContracts) { sum in
        ...
        .onReceive(vmContracts.$error, perform: { error in
            if error != nil {
                showAlert.toggle()
            }
        })
        .alert(isPresented: $showAlert) {
            Alert(title: Text("Error"), message: Text(vmContracts.error?.localizedDescription ?? ""))
        }
    }
    func findElement(location: CGPoint, proxy: ChartProxy, geometry: GeometryProxy) -> ContractsItem? {
        let relativeXPosition = location.x - geometry[proxy.plotAreaFrame].origin.x
        if let date = proxy.value(atX: relativeXPosition) as Date? {
            // Find the closest date element.
            var minDistance: TimeInterval = .infinity
            var index: Int? = nil
            for salesDataIndex in vmContracts.contracts.indices {
                let nthSalesDataDistance = vmContracts.contracts[salesDataIndex].date.distance(to: date)
            ...
            if let index {
                return vmContracts.contracts[index]
            }
        }
        return nil
    }
